...
class UserListEventsTestCase(TestCase):
    def setUp(self):
        ...
        self.start_date1 = date(2023, 7, 10)
        self.event1 = event.objects.create(
                ...
                END_TIME="09:00:00", online="t", CAPACITY="5000", STATUS="test")
    ...
    def test_user_list_events(self):
        ...
        self.url = reverse('user_list_events')
    ...
    def test_user_list_past_events(self):
        self.client.force_authenticate(user=self.user)
        url = reverse('user_list_past_events')
        response = self.client.get(url, follow=True)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        expected_data = eventSerializer([self.event1], many=True).data
        self.assertEqual(response.data, expected_data)

    def test_user_list_upcoming_events(self):
        self.client.force_authenticate(user=self.user)
        url = reverse('user_list_upcoming_events')
        response = self.client.get(url, follow=True)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        expected_data = eventSerializer([self.event2], many=True).data
        self.assertEqual(response.data, expected_data)
